...
- 仅更新当前分支的同名远端分支，并使用快进合并；分支分叉或没有共同历史会
  失败退出。仅显式传入 `--reset-on-divergence` 时，才会先创建
  `update-backup/<timestamp>-<branch>` 本地备份分支，再重置到远端提交。
- 脏检查忽略仅由父仓库提交切换造成的嵌套 gitlink 落后，并在确认嵌套子模块
  不含源码改动后，将其递归检出到父仓库锁定的提交；`.gitignore` 已排除的构建
  产物和生成的 `*.egg-info/` 元数据不阻塞更新，任何已跟踪或其他未忽略的源码
  改动仍会退出。
...
更新所有子模块。也可使用 `--repo /path/to/repo` 更新一个独立仓库。只有已确认
上游历史被重写且要放弃本地分支线时，使用 `./update.sh --reset-on-divergence`。
